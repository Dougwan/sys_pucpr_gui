...
class ButtonsGrid(QGridLayout):
    def __init__(self, items=List[List[str]], *args, **kwargs):
    ...
    def _get_grid_greater_column_span(self):
        ...
        for column_data in self._items:
    ...
    def _set_buttons_in_grid(self):
        ...
        for row, row_data in enumerate(self._items):
            for col, button_text in enumerate(row_data):

                button = Button(button_text.capitalize())
                self.addWidget(button, row, col, 1,
